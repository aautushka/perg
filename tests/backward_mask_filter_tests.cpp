...
struct backward_mask_filter_test : public ::testing::Test
{
	using result = std::vector<perg::view>;
	perg::pipeline<perg::view> pipeline;
};

TEST_F(backward_mask_filter_test, finds_a_matching_line_in_input_stream)
{
...
TEST_F(backward_mask_filter_test, finds_a_matching_stream_at_the_beginning)
{
	test_source source = {"hello world\nhow are you\ngood day"};
	test_sink sink; 
	perg::filters::backward_mask_filter filter("world");
...
TEST_F(backward_mask_filter_test, finds_a_match_at_the_end)
{
	test_source source = {"hello world\nhow are you\ngood day"};
	test_sink sink; 
	perg::filters::backward_mask_filter filter("good");
